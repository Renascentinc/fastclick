;(function () {
	...
	*/
	...
	FastClick.prototype.needsClick = function(target) {
		...
		function closest(target, fn) {
		  return target && (fn(target) ? target : closest(target.parentNode, fn));
		}
		// Check if target node or a parent node has "needsclick" class
		var reNeedsClick = closest(target, function(target){
			return (/\bneedsclick\b/).test(target.className);
		});

		return reNeedsClick ? true : false;
	};
